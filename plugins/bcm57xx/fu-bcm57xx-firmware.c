...
#define BCM_STAGE1_HEADER_MAGIC_MEKLORT		0x42434D1A
				  ...
				  GError **error)
{
	...
	g_debug ("stage1-size: 0x%x", (guint) stage1_sz);
	g_debug ("stage1-offset: 0x%x", (guint) stage1_off);
				  ...
				  GError **error)
{
	...
	g_debug ("stage2-offset: 0x%x", (guint) stage2_off);
	g_debug ("stage2-size: 0x%x", (guint) stage2_sz);
...
fu_bcm57xx_firmware_write (FuFirmware *firmware, GError **error)
{
	...
	fu_byte_array_append_uint32 (buf, fu_bcm57xx_nvram_crc (buf->data, buf->len, 0xffffffff), G_BIG_ENDIAN);
